# =============================================================================
# 云途 AI 行程规划 - 行程生成与编辑核心服务
# =============================================================================
# 这是整个后端最核心的模块，负责：
#   1. 生成完整 itinerary（generate_trip_itinerary）
#   2. 编辑已有 itinerary（edit_trip_itinerary）
#   3. 预算估算与拆分（_estimate_ticket_cost, _prorate_amounts 等）
#   4. 地图数据补全（_maybe_enrich_itinerary_with_map_data）
#
# 核心流程：
#   TripRequest → RAG检索上下文 → LLM生成草稿 → 规则补全 → 地图丰富 → Itinerary
# =============================================================================
...
def generate_trip_itinerary(request: TripRequest) -> Itinerary:
    ...
    logger.info(
        "[token_usage] Query Rewrite: prompt=%d, completion=%d",
        token_usage.rewrite_prompt_tokens,
        token_usage.rewrite_completion_tokens,
    )
    logger.info(
        "[token_usage] Rerank: prompt=%d, completion=%d",
        token_usage.rerank_prompt_tokens,
        token_usage.rerank_completion_tokens,
    )
    logger.info(
        "[token_usage] Query Embedding: prompt=%d, completion=%d",
        token_usage.embedding_prompt_tokens,
        token_usage.embedding_completion_tokens,
    )
    logger.info(
        "[token_usage] Planner: prompt=%d, completion=%d",
        token_usage.planner_prompt_tokens,
        token_usage.planner_completion_tokens,
    )
    logger.info(
        "[token_usage] Total: prompt=%d, completion=%d, all=%d",
