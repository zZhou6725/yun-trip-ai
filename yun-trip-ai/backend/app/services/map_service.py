# =============================================================================
# 云途 AI 行程规划 - 地图服务（高德地图）
# =============================================================================
# 封装高德地图 Web API 的以下能力：
#   - geocode_address：地理编码（地址 → 经纬度）
#   - search_places：POI 搜索（关键词 → 地点列表）
#   - estimate_route：路线规划（两点间距离/耗时）
#   - enrich_itinerary_with_map_data：批量补全 itinerary 中地点信息
# 所有外部 API 调用均支持 Redis 缓存，减少重复请求。
# =============================================================================
...
def geocode_address(address: str, city: str | None = None) -> dict[str, Any] | None:
    ...
    cached_value = get_cached_json(cache_key, CacheCategory.MAP)
    ...
    set_cached_json(cache_key, result, CacheCategory.MAP)
    return result
...
def search_places(
    ...
    set_cached_json(cache_key, results, CacheCategory.MAP)
    return results
