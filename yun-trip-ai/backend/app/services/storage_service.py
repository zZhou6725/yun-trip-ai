...
# 云途 AI 行程规划 - 行程存储服务
# =============================================================================
# 基于 SQLAlchemy + SQLite 提供 itinerary 的持久化 CRUD 操作：
#   - save_itinerary：保存或更新行程
#   - get_itinerary_by_trip_id：按 ID 查询
#   - list_saved_itineraries：列出所有行程
#   - delete_itinerary_by_trip_id：按 ID 删除
#   - get_token_stats：统计所有行程的 LLM Token 消耗
# =============================================================================
...
def save_itinerary(itinerary: Itinerary, user_id: int | None = None) -> str:
    """保存或更新完整 itinerary，关联指定用户，返回 trip_id。

    Args:
        itinerary: 完整行程数据
        user_id: 用户 ID（可选，用于关联行程与用户）
    """
    init_db()
