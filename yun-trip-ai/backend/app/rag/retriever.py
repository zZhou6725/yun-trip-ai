# =============================================================================
# 云途 AI 行程规划 - RAG 检索与重排序
# =============================================================================
# 负责：
#   1. 从向量库召回候选攻略片段
#   2. 使用 DashScope qwen3-rerank（Cross-encoder）做语义重排序
#   3. Rerank 不可用时回退到规则级打分重排序
# 所有检索和重排序结果均支持 Redis 缓存。
# =============================================================================
...
def _rerank_with_dashscope(
    ...
    try:
        logger.info("calling qwen3-rerank: query=%s, candidate_count=%d, top_k=%d",
                    query, len(clean_chunks), top_k)
        with httpx.Client(timeout=30) as client:
            ...
            logger.debug("qwen3-rerank status_code=%d", response.status_code)
            if response.status_code != 200:
                logger.warning("qwen3-rerank response preview=%s", response.text[:500])
                logger.warning(
        ...
        logger.info("qwen3-rerank token: prompt=%d, completion=%d, source=%s",
                    token_usage['prompt_tokens'], token_usage['completion_tokens'], usage_source)
        if not has_official_usage:
            logger.debug("qwen3-rerank response has no official usage field; precise token count is unavailable")
        ...
        if not results:
            logger.warning("qwen3-rerank empty results, response preview=%s",
        ...
        logger.info("qwen3-rerank success: results=%d", len(scored))
        logger.info("dashscope rerank: query=%s, results=%d", query, len(scored))
        return scored, token_usage

    except Exception as exc:
        logger.warning("qwen3-rerank failed: %s: %s", type(exc).__name__, exc)
...
def rerank_guide_chunks(
    ...
    cached = get_cached_json(cache_key, CacheCategory.RERANK)
    ...
    if dashscope_results:
        ...
        set_cached_json(cache_key, cache_value, CacheCategory.RERANK)
...
def retrieve_travel_guide(
    ...
    cached_value = get_cached_json(cache_key, CacheCategory.RAG)
    ...
    set_cached_json(cache_key, results, CacheCategory.RAG)
