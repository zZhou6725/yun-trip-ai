...
const anchors = [
  ...
  { id: "day-budget",label: "按天花费" },
  { id: "edit",      label: "智能调整" },
  { id: "map",       label: "景点地图" },
  { id: "weather",   label: "天气信息" },
  { id: "points",    label: "点位明细" },
  { id: "daily",     label: "每日行程" },
];
...
async function openPdfExport() {
  ...
  try {
    ...
    const blob = await fetchPdfExport(itineraryToExport.trip_id);
...
async function openMarkdownExport() {
  ...
  try {
    await saveTrip(itineraryToExport);
    const blob = await fetchMarkdownExport(itineraryToExport.trip_id);
...
<template>
  <section v-if="itinerary" class="result-page">
    <aside class="sidebar-card">
      ...
      <ul class="sidebar-list">
        <li v-for="anchor in anchors" :key="anchor.id"
            :class="{ 'sidebar-list__item--active': activeSection === anchor.id }"
            @click="scrollToSection(anchor.id)">{{ anchor.label }}</li>
      </ul>
    ...
    <div class="result-grid">
      <section id="overview" class="result-card">
      ...
      <section id="budget" class="result-card">
      ...
      <section id="map" class="result-card result-card--map">
        <div class="result-card__title">景点地图</div>
        <AmapTripMap :points="mapPoints" />
      </section>

      <section id="weather" class="result-card result-card--weather">
      ...
      <section id="edit" class="result-card result-card--full">
      ...
      <section id="day-budget" class="result-card result-card--full">
      ...
      <section id="points" class="result-card result-card--full">
      ...
      <section id="daily" class="result-card result-card--full">
